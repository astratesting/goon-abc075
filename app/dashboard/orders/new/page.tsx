...
export default function NewOrderPage() {
  ...
  function handleNext() {
    ...
    track("onboarding_step_completed", { step: step + 1, context: "new_order" });
    setStep(step + 1);
  }
    ...
    <div className="mx-auto max-w-3xl px-6 py-8 lg:px-8">
      ...
      {step === 0 && (
        <div className="space-y-6">
          <div>
            <h1 className="text-2xl font-bold text-gray-900">Upload your file</h1>
            <p className="mt-1 text-sm text-gray-500">
              Drop your 3D model to get an instant quote.
            </p>
          </div>
          <DropZone file={file} onFile={handleFile} onRemove={() => setFile(null)} error={fileError} />
          <p className="text-xs text-gray-400">
            Files stay private to your account. Stored for 30 days for fulfillment.
          </p>
          <div className="flex justify-end">
            <Button onClick={handleNext} disabled={!file}>Continue</Button>
          </div>
        </div>
      )}
      ...
      {step === 1 && (
        <div className="space-y-6">
          <div>
            <h1 className="text-2xl font-bold text-gray-900">Choose a material</h1>
            <p className="mt-1 text-sm text-gray-500">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            {MATERIALS.map((mat) => (
              ...
              >
                ...
                <p className="mt-1 text-xs text-gray-500 leading-relaxed">{mat.description}</p>
                <p className="mt-3 text-sm font-semibold text-sky-700">from ${mat.basePrice}</p>
              </button>
            ))}
          </div>
          <p className="text-xs text-gray-400">
          ...
          <div className="flex justify-between">
            ...
            <Button onClick={handleNext} disabled={!selectedMaterial}>Continue</Button>
          </div>
        </div>
      )}
      ...
      {step === 2 && quote && (
        <div className="space-y-6">
          <h1 className="text-2xl font-bold text-gray-900">Your instant quote</h1>
          ...
          <Card className="bg-sand-brand/10 border-sand-brand/30">
            <div className="space-y-4">
              ...
              <div className="border-t border-sand-brand/30 pt-4">
                ...
                <p className="text-3xl font-bold text-gray-900 mt-1">{formatCurrency(quote.total)}</p>
              </div>
              <div className="flex items-center gap-2 text-sm text-gray-500">
                <Clock className="h-4 w-4" />
                <span>Lead time: ~{quote.leadDaysMin}-{quote.leadDaysMax} days</span>
              </div>
          ...
          <div className="flex flex-col sm:flex-row justify-between gap-3">
            ...
            <Button onClick={handlePlaceOrder} loading={submitting}>Place order</Button>
          </div>

          {/* Sticky mobile CTA */}
          <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 p-4 sm:hidden">
            <Button onClick={handlePlaceOrder} loading={submitting} className="w-full py-3">
              Place order &mdash; {formatCurrency(quote.total)}
            </Button>
          </div>
        </div>
      )}
